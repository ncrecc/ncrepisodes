...
var specialfloor = addfloor('cool');
specialfloor.addenemies([level1enemies.pop(), level1enemies.pop(), level2enemies.pop(), level2enemies.pop()],[]);
specialfloor.generate();

var actfloorthing = new motion.actuators.SimpleActuator(null,0,null);
actfloorthing.repeat(-1);
var funcfloorthing = new hscript.Parser().parseString("
	if(floor == null || dummy == null) { actfloorthing._repeat = 0; }
	else {
		if(Rules.substitutions.exists(\"dolarry\") && dummy.innate.indexOf(\"didlarry\") == -1) {
			trace(\"found a larryizer\");
			dummy.innate.push(\"didlarry\");
			for (node in floor.nodes) {
				if (node.enemytemplate != null) {
					node.enemytemplate.name = \"Larry\"; 
				}
			}
		}
	}
...
interp.variables.set("dummy", dummy);
...
var floorone = addfloor("small")
  .additems(items, gooditems)
  .addotherstuff(otherstuff, goodotherstuff)
  .addenemies([level1enemies.pop()], [level1enemies.pop()])
  .generate();

trace(floorone.toString());

for(node in floorone.nodes) {
	trace(node.item);
	if(node.item != "") node.item = "Bop,Bop,Bop";
}
...
var commonlist2 = ["Healer Staff", "Gizmotron", "Gag Rule", "Cremator", "Sheer Will", "Revolver@ncrmod", "Tear Down This Wall", "Vanity Mirror", "Gyrate Hook", "Junk Sword", "Bumpbomb", "Heat Pump", scraplist.pop()];
